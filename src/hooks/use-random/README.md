...
- type 🎗️: could be individual, object or array and it determens the return type of the generate function so it could be a single word or a collection of words.
- amount 📶: if the type of the random string is selected as array or object then this prop determins the size of that collection.
- charset 🔠: the group of the characters the word will be created from. We provided some of the popular charsets to use but still you can provide your own custom charset.

The hook returns a single function to generate the random string depends on the options you gave to the hook.
